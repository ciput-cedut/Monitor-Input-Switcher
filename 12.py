...
class App(customtkinter.CTk):
    def __init__(self):
        ...
        title_label = customtkinter.CTkLabel(header_frame, text="Monitor Input Switcher", font=("Arial", 18, "bold"))
        ...
        monitor_section = customtkinter.CTkFrame(self.main_frame)
        monitor_section.pack(fill="x", pady=(0, 20))

        self.monitor_label = customtkinter.CTkLabel(monitor_section, text="📺 Monitor:", font=("Arial", 12, "bold"))
        self.monitor_label.pack(anchor="w", pady=(0, 8))

        self.monitor_menu = customtkinter.CTkOptionMenu(monitor_section, values=["Loading..."], command=self.update_inputs)
        self.monitor_menu.set("Loading...")
        self.monitor_menu.pack(fill="x", pady=0)

        # --- Input Source Section ---
        input_section = customtkinter.CTkFrame(self.main_frame)
        input_section.pack(fill="x", pady=(0, 20))

        self.input_label = customtkinter.CTkLabel(input_section, text="🔌 Input Source:", font=("Arial", 12, "bold"))
        self.input_label.pack(anchor="w", pady=(0, 8))

        self.input_menu = customtkinter.CTkOptionMenu(input_section, values=["Loading..."])
        self.input_menu.set("Loading...")
        self.input_menu.pack(fill="x", pady=0)
        ...
        self.switch_button = customtkinter.CTkButton(button_frame, text="⚡ Switch Input", command=self.switch_input, height=40, font=("Arial", 12, "bold"))
        self.switch_button.pack(side="left", padx=(0, 10), expand=True, fill="both")

        self.refresh_button = customtkinter.CTkButton(button_frame, text="🔄 Refresh", command=self.refresh_monitors, height=40, font=("Arial", 12, "bold"))
        self.refresh_button.pack(side="right", padx=(10, 0), expand=True, fill="both")

        # --- Favorites Section ---
        favorites_header_frame = customtkinter.CTkFrame(self.main_frame)
        favorites_header_frame.pack(fill="x", pady=(20, 10))

        self.favorites_label = customtkinter.CTkLabel(favorites_header_frame, text="⭐ Quick Favorites", font=("Arial", 12, "bold"))
        self.favorites_label.pack(side="left", anchor="w")

        self.manage_favorites_btn = customtkinter.CTkButton(favorites_header_frame, text="+ Manage", command=self.show_manage_favorites, state="disabled", width=100, height=28, font=("Arial", 12))
        self.manage_favorites_btn.pack(side="right")

        self.favorites_frame = customtkinter.CTkFrame(self.main_frame)
        self.favorites_frame.pack(fill="x", pady=(0, 20))

        # ===== CONTROL BUTTONS FRAME =====
        self.control_frame = customtkinter.CTkFrame(self)
        self.control_frame.pack(padx=20, pady=(0, 15), fill="x")

        self.shortcuts_button = customtkinter.CTkButton(self.control_frame, text="⌨️ Keyboard Shortcuts", command=self.show_shortcuts_editor, state="disabled", font=("Arial", 12))
        self.shortcuts_button.pack(side="left", padx=(0, 10), expand=True, fill="x")

        self.theme_button = customtkinter.CTkButton(self.control_frame, text="🎨 Theme", command=self.show_theme_settings, font=("Arial", 12))
        self.theme_button.pack(side="right", padx=(10, 0), expand=True, fill="x")

        # ===== STATUS & FOOTER FRAME =====
        footer_frame = customtkinter.CTkFrame(self)
        footer_frame.pack(fill="x", padx=20, pady=(0, 15))

        self.status_label = customtkinter.CTkLabel(footer_frame, text="", wraplength=560, font=("Arial", 12))
        self.status_label.pack(pady=(0, 10))

        self.name_label = customtkinter.CTkLabel(footer_frame, text="By: LuqmanHakimAmiruddin@PDC", font=("Arial", 9), text_color="gray")
